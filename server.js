...
const donhangRouter = require("./route/donhang.js");
...
app.get("/", function (request, response) {
  ...
  fs.readFile(filePath, "utf8", function (err, data) {
    ...
    data = data.replace(
      /\$OG_DESCRIPTION/g,
      "VUA ĐỒ GỖ chuyên cung cấp đồ gỗ nội thất, đồ gỗ về phòng ngủ,phòng bếp,phòng thờ,phòng khách...VUA ĐỒ GỖ, GỖ THẬT GIÁ TRỊ THẬT."
    );
    data = data.replace(
      /\$OG_KEYWORD/g,
      "vua đồ gỗ, vua do go, đồ gỗ nội thất, đồ gỗ phòng ngủ, do go noi that, đồ gỗ phòng khách, đồ gỗ,đồ gỗ tốt"
    );
...
app.post("/api/uploads", middleware.checkToken, (req, res) => {
  ...
  for (var key in req.files) {
    req.files[key].mv(
      path.join(__dirname, "build", "images", req.files[key].name)
    );
...
app.post("/api/info", middleware.checkToken, (req, res) => {
  ...
  const html = `<div>
            ...
            ${req.body.cart
              .map((p, key) => {
                return `<tr>
                        ...
                        <td>${p.name}</td>
                        <td>${p.soluong}</td>
                        <td style="color:red" >${new Intl.NumberFormat().format(
                          p.price
                        )} VNĐ</td>
                        <td style="color:red">${new Intl.NumberFormat().format(
                          p.price * p.soluong
        ...
        <b>Tổng tiền: </b> <span style="color:red">${new Intl.NumberFormat().format(
          total
        )} VNĐ</span>
    </div>`;
